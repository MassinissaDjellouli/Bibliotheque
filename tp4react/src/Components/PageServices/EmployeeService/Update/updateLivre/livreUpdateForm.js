...
export const LivreUpdateForm = ({ submit, livre }) => {
    ...
    const onSubmit = (e) => {
        let tempsEmprunt = 3;
        const updatedLivre = { titre, auteur, editeur, anneeDePublication, tempsEmprunt, nbExemplaires, nbPages, genre }
        e.preventDefault()
        submit(updatedLivre,livre.documentId)
    }
    if(livre.titre == undefined){
        return <>
            <EmployeHeader />
            <h1 className='text-center text-danger'>Pas de livre</h1>
        </>
    }
    return (
        <>
            <EmployeHeader />
            <h1 className='text-center'>Modifier un livre</h1>
            <div className='p-5'>
                <DocumentCard document={livre} />
            </div>
            <form className='container pb-4' onSubmit={onSubmit} >
                <div>
