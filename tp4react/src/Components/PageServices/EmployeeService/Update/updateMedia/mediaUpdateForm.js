...
export const MediaUpdateForm = ({ submit, livre: media }) => {
    ...
    const onSubmit = (e) => {
        ...
        const mediaUpdated = { titre, auteur, editeur, anneeDePublication, tempsEmprunt, nbExemplaires, duree, type }
        e.preventDefault()
        submit(mediaUpdated,media.documentId)
    }
    if(media.titre == undefined){
        return <>
            <EmployeHeader />
            <h1 className='text-center text-danger'>Pas de média</h1>
        </>
    }
    return (
        <>
            <EmployeHeader />
            <h1 className='text-center'>Modifier un média</h1>
            <div className='p-5'>
                <DocumentCard document={media} />
            </div>
            <form className='container pb-5' onSubmit={onSubmit} >
            <div>
